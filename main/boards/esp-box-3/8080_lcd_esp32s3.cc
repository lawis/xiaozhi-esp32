...
static void lcd_set_pin(const i2s_lcd_config_t *config)
{
    ...
    gpio_set_direction((gpio_num_t)config->pin_num_wr, GPIO_MODE_OUTPUT);
    gpio_set_pull_mode((gpio_num_t)config->pin_num_wr, GPIO_FLOATING);
    ...
    for (int i = 0; i < config->data_width; i++) {
        ...
        gpio_set_direction((gpio_num_t)config->pin_data_num[i], GPIO_MODE_OUTPUT);
        gpio_set_pull_mode((gpio_num_t)config->pin_data_num[i], GPIO_FLOATING);
        gpio_pad_set_drv((gpio_num_t)config->pin_data_num[i], 0);
        gpio_matrix_out((gpio_num_t)config->pin_data_num[i], LCD_DATA_OUT0_IDX + i, false, false);
    }
}
...
i2s_lcd_handle_t i2s_lcd_driver_init(const i2s_lcd_config_t *config)
{
    ...
    if (config->pin_num_cs >= 0) {
        ...
        gpio_set_direction((gpio_num_t)config->pin_num_cs, GPIO_MODE_OUTPUT);
        gpio_pad_set_drv((gpio_num_t)config->pin_num_cs, 0);
        gpio_set_level((gpio_num_t)config->pin_num_cs, 0);
    }

    gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[config->pin_num_rs], PIN_FUNC_GPIO);
    gpio_pad_select_gpio((gpio_num_t)config->pin_num_rs);
    gpio_set_direction((gpio_num_t)config->pin_num_rs, GPIO_MODE_OUTPUT);
    gpio_pad_set_drv((gpio_num_t)config->pin_num_rs, 0);
    gpio_set_level((gpio_num_t)config->pin_num_rs, LCD_DATA_LEV);
...
esp_err_t i2s_lcd_write_cmd(i2s_lcd_handle_t handle, uint16_t cmd)
{
    i2s_lcd_driver_t *i2s_lcd_drv = (i2s_lcd_driver_t *)handle;
    LCD_CHECK(NULL != i2s_lcd_drv, "handle pointer invalid", ESP_ERR_INVALID_ARG);
    gpio_set_level((gpio_num_t)i2s_lcd_drv->rs_io_num, LCD_CMD_LEV);
    lcd_write_data(i2s_lcd_drv->i2s_lcd_obj, (uint8_t *)&cmd, i2s_lcd_drv->i2s_lcd_obj->width == 16 ? 2 : 1);
    gpio_set_level((gpio_num_t)i2s_lcd_drv->rs_io_num, LCD_DATA_LEV);
    return ESP_OK;
}
