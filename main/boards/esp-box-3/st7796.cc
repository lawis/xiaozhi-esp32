...
scr_driver_t lcd_st7796_default_driver = {
    ...
    .init_reg = NULL,
    .write_cmd = lcd_st7796_write_cmd,
    .write_data = lcd_st7796_write_data,
};
...
esp_err_t lcd_st7796_init(const scr_controller_config_t *lcd_conf)
{
    ...
    if (lcd_conf->pin_num_rst >= 0) {
        ...
        gpio_set_direction((gpio_num_t)lcd_conf->pin_num_rst, GPIO_MODE_OUTPUT);
        gpio_set_level((gpio_num_t)lcd_conf->pin_num_rst, (lcd_conf->rst_active_level) & 0x1);
    ...
    if (lcd_st7796_default_driver.init_reg) {
        ret = lcd_st7796_default_driver.init_reg();
    } else {
    ...
    if (lcd_conf->pin_num_bckl >= 0) {
        gpio_pad_select_gpio((gpio_num_t)lcd_conf->pin_num_bckl);
        gpio_set_direction((gpio_num_t)lcd_conf->pin_num_bckl, GPIO_MODE_OUTPUT);
        gpio_set_level((gpio_num_t)lcd_conf->pin_num_bckl, (lcd_conf->bckl_active_level) & 0x1);
    }
...
esp_err_t lcd_st7796_write_cmd(uint16_t cmd) {
    return LCD_WRITE_CMD(cmd);
}

esp_err_t lcd_st7796_write_data(uint16_t data) {
    return LCD_WRITE_DATA(data);
}
