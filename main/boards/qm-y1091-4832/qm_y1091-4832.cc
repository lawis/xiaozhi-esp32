...
class QmY10914832Board : public WifiBoard
{
private:
    ...
    void InitializeAw9523() {
        ...
        aw9523_ = new Aw9523(i2c_bus_, 0xB2 >> 1); //0x59
        vTaskDelay(pdMS_TO_TICKS(50));
    }

    void InitializeFt5x06() {
        ESP_LOGI(TAG, "Init FT5x06");
        ft5x06_ = new FT5x06(i2c_bus_, 0x38, -1, 6); //0x38
        vTaskDelay(pdMS_TO_TICKS(50));
    }
...
public:
    QmY10914832Board() : boot_button_((gpio_num_t)42)
    {
        // InitializeI2c();
        // InitializeSpi();

        ESP_LOGI(TAG, ">>>>>>>>>>>>> QmY10914832Board");
        InitializeI2c();
        InitializeAw9523();
        I2cDetect();
        gpio_install_isr_service(ESP_INTR_FLAG_SHARED);

        display_ = new QmY1091Display(DISPLAY_WIDTH, DISPLAY_HEIGHT, DISPLAY_OFFSET_X, DISPLAY_OFFSET_Y, DISPLAY_MIRROR_X, DISPLAY_MIRROR_Y, DISPLAY_SWAP_XY);

        // display_->SetupUI();
        InitializeFt5x06();
        display_->initTouchPanel(ft5x06_);
        // InitializeIli9341Display();
        // InitializeButtons();
        InitializeIot();
        ESP_LOGI(TAG, ">>>>>>>>>>>>> QmY10914832Board 2");
    }
...
DECLARE_BOARD(QmY10914832Board);
