...
public:
    Lamp3() : Thing("Lamp3", "办公区的吸顶灯"), power_(false) {
        InitializeGpio();
        ...
        methods_.AddMethod("TurnOn", "打开灯", ParameterList(), [this](const ParameterList& parameters) {
            ...
            ESP_LOGI(TAG, "打开办公区灯");
            HaClient::GetInstance().post_services_light_toggle("light.tze200_do3lhj3y_ts0601_deng_guang", true);
        });

        methods_.AddMethod("TurnOff", "关闭灯", ParameterList(), [this](const ParameterList& parameters) {
            power_ = false;
            ESP_LOGI(TAG, "关闭办公区灯");
            HaClient::GetInstance().post_services_light_toggle("light.tze200_do3lhj3y_ts0601_deng_guang", false);
